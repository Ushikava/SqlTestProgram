Брюханов А.В. Выполнение тестового задания
...
Для разработки использован язык программирования C#, а для создания базы данных PostgreSQL.

Каждое из пунктов задания представлено в виде методов класса, которые можно вызвать при работе с приложением.

Для ускорения работы пятого задания, в шестом, для решения проблемы ускорения, были использованы индексы.
При работе с ними, при первом создании и обработке индекса, поиск будет выполняется за долгий промежуток времени.
Однако, при повторном поиске среди миллиона элементов необходимых по запросу, операция будет производиться в разы быстрее.

Приведенное время работы алгоритма поиска без индекса: 2 секунды и 576 миллисекунд (на моей машине)
При повторном вызове с уже созданным индексом - 1 секунда и 184 миллисекунды (так же на моей машине)

Время работы запросов из пунктов 5 и 6 выводится при работе с приложением.